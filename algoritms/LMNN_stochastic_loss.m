function [M,loss,gradient,Mnorm,time] = LMNN_stochastic_loss(X, y, K, mu, maxIter, alpha, tol, samples)
%% Description
...
if(nargin < 7),     samples = min([100,size(X,1)]); end
...
for iter = 1:maxIter
    tic;
    iter
    loss(iter+1) = lossFunction(X, y, tN, mu, M);    
    
    % Calculate triplets under the current metric
    T = findTriples_stochastic(X, y, tN, M, samples);
    nbTriplets = size(T,1);
